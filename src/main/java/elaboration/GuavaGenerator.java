...
public class GuavaGenerator extends GuavaBaseVisitor<String> {
    ...
    /** List of ParseTrees that cannot be emptied by the {@link #emptyReg(ParseTree) emptyReg} method.*/
    private List<ParseTree> persistent;
...
//    private static final String IND = "IND";
    ...
    public String visitForStat(GuavaParser.ForStatContext ctx) {
        ...
        setCodeLines(ctx, lines);
        setPersistent(ctx.expr(0), false);
    ...
    public String visitParExpr(GuavaParser.ParExprContext ctx) {
        visit(ctx.expr());
        setRegExplicit(ctx, reg(ctx.expr()));
    ...
    private void setPersistent(ParseTree tree, boolean persistent) {
        if (persistent) {
            if (!this.persistent.contains(tree)) {
                this.persistent.add(tree);
            }
        } else {
            if (this.persistent.contains(tree)) {
                this.persistent.remove(tree);
            }
        }

    }

    private boolean isPersistent(ParseTree tree) {
        return this.persistent.contains(tree);
    }
